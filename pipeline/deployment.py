...
def deploy(output_dir=None, audio_path=None,checkpoint_path=None):
    ...
    if output_dir is None:base_out = Path(config.config["RESULTS_ROOT"]) / "deployment" / loss_fn / f"{num_sources}stems"
    else: base_out = Path(output_dir)
    ...
    if audio_name: graph_name = audio_name.replace(".wav", "_sources.png")
